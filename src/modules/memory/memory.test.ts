...
describe("isPromoActive", () => {
  ...
  it("uses the tenant's today, not UTC's", () => {
    // 01:30 UTC on the 1st is already past midnight in Stockholm (the
    // default), so the promo the business wrote "till den 30:e" has ended —
    // but it is still 22:30 on the 30th in Asunción, so a tenant on that
    // timezone sees it still running that evening.
    const lateEvening = new Date("2026-10-01T01:30:00Z");
    expect(isPromoActive(promo({ validUntil: "2026-09-30" }), lateEvening)).toBe(false);
    expect(
      isPromoActive(promo({ validUntil: "2026-09-30" }), lateEvening, "America/Asuncion"),
    ).toBe(true);
  });
});
