...
export const bookingTypes = mysqlTable(
  "booking_types",
  {
    ...
    /**
     * The deposit this type asks for, in minor units (öre for SEK, plan.md
     * §1.2) — the same money convention every other amount in the app uses,
     * never a float. Null means no deposit and the booking confirms
     * immediately.
     */
    depositAmount: bigint("deposit_amount", { mode: "number" }),
    depositCurrency: varchar("deposit_currency", { length: 3 }).notNull().default("SEK"),
...
export const bookingTypeServices = mysqlTable(
  "booking_type_services",
  {
    ...
    /** Minor units, same convention as `booking_types.deposit_amount`. */
